...
<body>
  <div class="wrap">
    <div class="meta">
      <div><strong>Name:</strong> <span id="vbName"></span></div>
      <div><strong>Version:</strong> <span id="vbVersion"></span></div>
      <div><strong>Vendor:</strong> <span id="vbVendor"></span></div>
      <div><strong>Style:</strong> <span id="styleName"></span></div>
    </div>

    <div class="row">
      <label>Target Magnitude</label>
      <input id="magSlider" type="range" min="-${range}" max="${range}" step="0.01" value="${payload.initial.toFixed(
      3
    )}" />
      <input id="magInput" type="number" step="0.01" value="${payload.initial.toFixed(3)}" />
      <div id="magError" class="error"></div>
    </div>

    <div class="hex-grid">
      <div>
        <label>Original Data</label>
        <div id="origHex" class="hex-box"></div>
      </div>
      <div>
        <label>Updated Preview</label>
        <div id="nextHex" class="hex-box"></div>
      </div>
    </div>
  ...
  <script nonce="${nonce}">
    ...
    const errorEl = document.getElementById("magError");
    const origHexEl = document.getElementById("origHex");
    const nextHexEl = document.getElementById("nextHex");
    const vbNameEl = document.getElementById("vbName");
    ...
    applyBtn.addEventListener("click", () => {
      ...
      vscode.postMessage({ type: "apply", value: input.value });
    });
    ...
    function hexToVec32(hex) {
      if (!/^[0-9a-fA-F]{256}$/.test(hex)) {
    ...
    function setMagnitude(vec, target) {
      const mag = l2Magnitude(vec);
      if (mag === 0) {
        return new Float32Array(vec.length);
      }
      const isNegZero = Object.is(target, -0);
      const sign = target < 0 || isNegZero ? -1 : 1;
      const scale = (Math.abs(target) * sign) / mag;
